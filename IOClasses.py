...
# BEagleJack, 2021-Mar-14, Added ability to save and load files
#------------------------------------------#
                ...
                for disc in lst_Inventory:
                    for track in disc.cd_tracks:
                        if track is not None:
                            all_tracks += str(disc.cd_id) + ',' + track.get_record()
                file.write(all_tracks)    
    ...
    def load_inventory(file_names: list) -> list:
        ...
        try:
            ...
            with open(file_names[1], 'r') as file:
                for line in file:
                    data = line.strip().split(',')
                    row = DC.Track(data[1], data[2], data[3])
